    ...
    def event_detail(self, event_id: int) -> dict[str, Any] | None:
        with sqlite3.connect(self.database_path) as connection:
            ...
            event = connection.execute(
                "SELECT * FROM normalized_events WHERE id = ?",
                (event_id,),
            ).fetchone()
            if not event:
                return None
    ...
    def settlements_search(self, query: str, limit: int = 20) -> list[dict[str, Any]]:
        ...
        with sqlite3.connect(self.database_path) as connection:
            ...
            rows = connection.execute(
                ...
                WHERE s.name_he LIKE ? OR COALESCE(s.name_en, '') LIKE ? OR COALESCE(sa.alias, '') LIKE ?
                ORDER BY s.name_he
                LIMIT ?
                """,
    ...
    def _latest_early_warning_for_settlement(self, settlement_id: int) -> dict[str, Any] | None:
        with sqlite3.connect(self.database_path) as connection:
            ...
            row = connection.execute(
                ...
                WHERE ne.normalized_type = 'early_warning' AND el.settlement_id = ?
